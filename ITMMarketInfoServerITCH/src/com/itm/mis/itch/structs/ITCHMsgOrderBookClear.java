...
public class ITCHMsgOrderBookClear extends ITCHMsgBase {
    ...
    public boolean parseMessage(byte[] btMessageBytes) {
        ...
        try {
            if (super.parseMessage(btMessageBytes)) {
                if (btMessageBytes.length >= 9 + SoupBinTCPOffset.OFFSET_FIELD_PAYLOAD) { // 1 + 4 + 4 = 9
                    ...
                    ITMFileLoggerManager.getInstance.insertLog(this, logSource.ITCH, logLevel.ERROR,
                            "Insufficient bytes for ITCHMsgOrderBookClear");
                }
            }
        } catch (Exception ex0) {

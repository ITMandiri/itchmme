...
public class ITCHMsgOrderBookState extends ITCHMsgBase {
    ...
    public boolean parseMessage(byte[] btMessageBytes) {
        ...
        try {
            if (super.parseMessage(btMessageBytes)) {
                if (btMessageBytes.length >= 29 + SoupBinTCPOffset.OFFSET_FIELD_PAYLOAD) { // 1 + 4 + 4 + 20 = 29
                    ...
                    setStateName(decodeString(btMessageBytes, SoupBinTCPOffset.OFFSET_FIELD_PAYLOAD + 9, 20));
                    mOut = true;
                } else {
                    ITMFileLoggerManager.getInstance.insertLog(this, logSource.ITCH, logLevel.ERROR,
                            "Insufficient bytes for ITCHMsgOrderBookState");
                }
            }
        } catch (Exception ex0) {
    ...
    public byte[] buildMessage() {
        byte[] mOut = resetCumulativeBytes()
                ...
                .concatenateField(getStateName(), SoupBinTCPOffset.OFFSET_FIELD_PAYLOAD + 9, 20)
                .putPacketLength()
                .getCumulativeBytes();
        return mOut;
    }

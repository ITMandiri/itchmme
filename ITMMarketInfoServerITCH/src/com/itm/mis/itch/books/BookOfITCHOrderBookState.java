 ...
 * @author Ari Pambudi
 */
public class BookOfITCHOrderBookState extends BookOfITCHBase {
    //.single instance:
    public final static BookOfITCHOrderBookState getInstance = new BookOfITCHOrderBookState();
    
    private final ConcurrentHashMap<Integer, SheetOfITCHOrderBookState> chmDirectory = new ConcurrentHashMap<>();
    
    public BookOfITCHOrderBookState() {
        //.nothing todo here :)
        ITMFileLoggerManager.getInstance.insertLog(this, logSource.ITCH, logLevel.INIT, "");
    }
    
    public boolean addOrUpdateSheet(SheetOfITCHOrderBookState mSheet){
        boolean mOut = false;
        try{
    ...
    public ConcurrentHashMap<Integer, SheetOfITCHOrderBookState> retrieveAllSheets(){
        return this.chmDirectory;
    }
    
    public SheetOfITCHOrderBookState retrieveSheet(int vOrderBookId){
        SheetOfITCHOrderBookState mOut = null;
        try{

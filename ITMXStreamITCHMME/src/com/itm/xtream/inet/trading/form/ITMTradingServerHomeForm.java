...
public class ITMTradingServerHomeForm extends javax.swing.JFrame {
    ...
    public final static String Z_REF_CONNECTION_TYPE_ITCH_MDF                   = "ITCH_MDF";
...
//            ConcurrentHashMap<String, ITMSoupBinTCPITCHMDFPacketController> chmITCHMDFs = ITMSoupBinTCPITCHMDFPacketMgr.getInstance.getAllConnectionLines();
            ...
            int pCurRow = 0;
            String zCurMsg = "";
            if ((chmITCHs != null) && (!chmITCHs.isEmpty())){
                for(ITMSoupBinTCPITCHPacketController mEachCtl : chmITCHs.values()){
                    if ((mEachCtl != null) && (!mEachCtl.getConnectionName().isEmpty())){
                        ...
                        if (mEachCtl.getChannel() != null){
                            zCurMsg = (mEachCtl.getChannel().isConnected() ? ((mEachCtl.isAuthLastAccepted()) ? "+LoggedOn" : "+Connected") : "-Disconnected") + ((mEachCtl.getStsBridgeStatus() == ITCHBridgeStatus.SCK_CONNECTING || mEachCtl.getStsBridgeStatus() == ITCHBridgeStatus.SCK_DISCONNECTING) ? " [" + mEachCtl.getStsBridgeStatus().cname + "]" : "");
                        }
                        ...
                        try{ this.jTableIdxConnectionList.setValueAt(Z_REF_CONNECTION_TYPE_ITCH, pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_TYPE));}catch(Exception ex0){}
...
//            if ((chmITCHMDFs != null) && (!chmITCHMDFs.isEmpty())){
//                for(ITMSoupBinTCPITCHMDFPacketController mEachCtl : chmITCHMDFs.values()){
//                    if ((mEachCtl != null) && (!mEachCtl.getConnectionName().isEmpty())){
//                        boolean bSetConnSelect = false;
//                        if (bMayFirstTimeFill){
//                            bSetConnSelect = mEachCtl.isAutoSelect();
...
//                        zCurMsg = "-";
//                        if (mEachCtl.getChannel() != null){
//                            zCurMsg = (mEachCtl.getChannel().isConnected() ? ((mEachCtl.isAuthLastAccepted()) ? "+LoggedOn" : "+Connected") : "-Disconnected") + ((mEachCtl.getStsBridgeStatus() == ITMSoupBinTCPITCHMDFPacketController.ITCHBridgeStatus.SCK_CONNECTING || mEachCtl.getStsBridgeStatus() == ITMSoupBinTCPITCHMDFPacketController.ITCHBridgeStatus.SCK_DISCONNECTING) ? " [" + mEachCtl.getStsBridgeStatus().cname + "]" : "");
//                        }
//                        ITMComponentLayoutHelper.addTableEmptyRows(this.jTableIdxConnectionList, 1);
//                        try{ this.jTableIdxConnectionList.setValueAt(bSetConnSelect, pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_SELECT));}catch(Exception ex0){}
//                        try{ this.jTableIdxConnectionList.setValueAt(mEachCtl.getConnectionName(), pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_NAME));}catch(Exception ex0){}
//                        try{ this.jTableIdxConnectionList.setValueAt(Z_REF_CONNECTION_TYPE_ITCH_MDF, pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_TYPE));}catch(Exception ex0){}
//                        try{ this.jTableIdxConnectionList.setValueAt(mEachCtl.getIPAddress(), pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_ADDRESS));}catch(Exception ex0){}
//                        try{ this.jTableIdxConnectionList.setValueAt(mEachCtl.getPort(), pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_PORT));}catch(Exception ex0){}
//                        try{ this.jTableIdxConnectionList.setValueAt(mEachCtl.getCurrentSequencedNo(), pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_SEQUENCE));}catch(Exception ex0){}
//                        try{ this.jTableIdxConnectionList.setValueAt(zCurMsg, pCurRow, ITMComponentLayoutHelper.getTableColumnIndex(this.jTableIdxConnectionList, Z_REF_TABLE_COLUMN_MESSAGE));}catch(Exception ex0){}
            ...
            if ((mCmpCheck != null) && (!mCmpCheck.isEmpty())){
                for(Object oConnName : mCmpCheck.keySet()){
                    ...
                    if ((zConnName != null) && (!zConnName.isEmpty())){
                        if (chmITCHs.containsKey(zConnName)){
                            if (bAsync){
                                //.pakai executor:
                                execConnectionService.execute(new Runnable() {
                                    ...
                                    public void run() {
                                        try{
                                            ITMSoupBinTCPITCHPacketMgr.getInstance.doConnectLine(false, zConnName);
                                        }catch(Exception ex0){
                                            ITMFileLoggerManager.getInstance.insertLog(this, logSource.XTTS, logLevel.ERROR, ex0);
                                        }
                                    }
...
//                        else if (chmITCHMDFs.containsKey(zConnName)){
//                            if (bAsync){
//                                //.pakai executor:
//                                execConnectionService.execute(new Runnable() {
//                                    @Override
//                                    public void run() {
//                                        try{
//                                            ITMSoupBinTCPITCHMDFPacketMgr.getInstance.doConnectLine(false, zConnName);
//                                        }catch(Exception ex0){
//                                            ITMFileLoggerManager.getInstance.insertLog(this, logSource.XTTS, logLevel.ERROR, ex0);
//                                        }
//                                    }
                        ...
                        if (chmITCHs.containsKey(zConnName)){
                            if (bAsync){
                                ...
                                execConnectionService.execute(new Runnable() {
                                    ...
                                    public void run() {
                                        try{
                                            ITMSoupBinTCPITCHPacketMgr.getInstance.doDisconnectLine(false, zConnName);
                                        }catch(Exception ex0){
                                            ITMFileLoggerManager.getInstance.insertLog(this, logSource.XTTS, logLevel.ERROR, ex0);
                                        }
                                    }
...
//                                    public void run() {
//                                        try{
//                                            ITMSoupBinTCPITCHMDFPacketMgr.getInstance.doDisconnectLine(false, zConnName);
//                                        }catch(Exception ex0){
//                                            ITMFileLoggerManager.getInstance.insertLog(this, logSource.XTTS, logLevel.ERROR, ex0);
//                                        }
//                                    }
...
//                        }
    ...
    private void formWindowClosing(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosing
        ...
        try{
            ...
            ITMSoupBinTCPITCHPacketMgr.getInstance.doDisconnectLine(true, "");

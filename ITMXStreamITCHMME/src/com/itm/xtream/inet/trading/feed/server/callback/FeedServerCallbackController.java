 ...
 * @author fredy
 */
public class FeedServerCallbackController extends ITMSocketCtrlServer {
    //.single instance ya:
    public static final String SOCKET_CONNECTION_NAME_AS_OUTPUT_OF_DATAFEED     = "DFC_OUT";
    public final static FeedServerCallbackController getInstance = new FeedServerCallbackController(SOCKET_CONNECTION_NAME_AS_OUTPUT_OF_DATAFEED);
    
    private int HostPort;
    
    private FeedServerCallbackHandler eventHandler;
    
    //.daftar saja, bukan urutan:
    private ConcurrentHashMap<ITMSocketChannel, FeedServerCallbackProcessor> activeChannelsProcessors = new ConcurrentHashMap<>(); //.daftar channel dan processor.<Channel,FeedServerCallbackProcessor>
    
    public FeedServerCallbackController(String zConnectionName) {
        
        super(zConnectionName);
        this.eventHandler = new FeedServerCallbackHandler(this);
    ...
    public ConcurrentHashMap<ITMSocketChannel, FeedServerCallbackProcessor> getAllChannelsProcessorsList(){
    ...
    public boolean isChannelsProcessorsOnLimit(){
        boolean mOut = false;
        if (this.activeChannelsProcessors.size() >= ITMTradingServerSettingsMgr.getInstance.getSettings().server_settings.datafeed_client.maxcount){
            mOut = true;
        }
        return mOut;
    }
    ...
    public void createChannelProcessor(ITMSocketChannel mChannel){
        try{
            if (!this.activeChannelsProcessors.containsKey(mChannel)){
                FeedServerCallbackProcessor mProcessor = new FeedServerCallbackProcessor();
                mProcessor.setChChannel(mChannel);
                mProcessor.startProcessor();
                this.activeChannelsProcessors.put(mChannel, mProcessor);
            }else{
                FeedServerCallbackProcessor mProcessor = this.activeChannelsProcessors.get(mChannel);
                mProcessor.setChChannel(mChannel);
                mProcessor.startProcessor();
            }
        } catch (Exception ex0) {
    ...
    public FeedServerCallbackProcessor getChannelProcessor(ITMSocketChannel mChannel){
        FeedServerCallbackProcessor mOut = null;
        try{
            if (this.activeChannelsProcessors.containsKey(mChannel)){

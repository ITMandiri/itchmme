 ...
 * @author fredy
 */
public class ITMSoupBinTCPOUCHPacketController extends ITMSoupBinTCPBridgeSocketCtl {
    
    public enum OUCHBridgeStatus{ //.connecting, connected, disconnecting, disconnected.
        ...
        private OUCHBridgeStatus(int order, String cname) {
            this.order = order;
            this.cname = cname;
        }
    ...
    private OUCHBridgeStatus stsBridgeStatus                     = OUCHBridgeStatus.SCK_DISCONNECTED;
    
    public OUCHBridgeStatus getStsBridgeStatus() {
        if (this.stsBridgeStatus == null){
            this.stsBridgeStatus = OUCHBridgeStatus.SCK_DISCONNECTED;
        }
        return this.stsBridgeStatus;
    }

    public void setStsBridgeStatus(OUCHBridgeStatus stsBridgeStatus) {
        this.stsBridgeStatus = stsBridgeStatus;
    }
    
    private final AtomicBoolean bMustConnected                      = new AtomicBoolean(false);
    
    private final ITMSoupBinTCPOUCHPacketListener evHandler;
    private OUCHInBridgeSocketGuardWorker recvGdWorker;
    ...
    public ITMSoupBinTCPOUCHPacketController(
        ...
        this.evHandler = new ITMSoupBinTCPOUCHPacketListener();
        ...
        try{
            if (this.bMustConnected.get()){
                ...
                if (recvGdWorker == null){
                    recvGdWorker = new OUCHInBridgeSocketGuardWorker();
                }
                if (recvGdWorker != null){
    ...
    public void doStartConnection(){
        try{
            ...
            if (!isConnected()){
                //.set (awal) status:
                stsBridgeStatus = OUCHBridgeStatus.SCK_CONNECTING;
                //.coba koneksi:
            ...
            //.set (ulang) status:
            if (bOut){
                stsBridgeStatus = OUCHBridgeStatus.SCK_CONNECTED;
            }else{
                stsBridgeStatus = OUCHBridgeStatus.SCK_DISCONNECTED;
            }
        }catch(Exception ex0){
    ...
    public void doStopConnection(){
        try{
            if (isConnected()){
                //.set (awal) status:
                stsBridgeStatus = OUCHBridgeStatus.SCK_DISCONNECTING;
            }
            //.beri tanda terlebih dulu:
    ...
    //.worker:
    private class OUCHInBridgeSocketGuardWorker implements Runnable{
                ...
                try{
                    ...
                    if (bMustConnected.get()){
                        if (!isConnected()){
                            ...
                            connect(getIPAddress(), getPort(), getTimeOut());
                            //.cek status:
                            stsBridgeStatus = (isConnected() ? OUCHBridgeStatus.SCK_CONNECTED : OUCHBridgeStatus.SCK_DISCONNECTED );
                        }
                    }else{
                        if (isConnected()){
                            //.set (awal) status:
                    ...
                    //.EXXX.
                    ITMFileLoggerManager.getInstance.insertLog(this, ITMFileLoggerVarsConsts.logSource.OUCH, ITMFileLoggerVarsConsts.logLevel.ERROR, intex0);
                    break;
                }catch(Exception ex0){
                    //.EXXX.
                    ITMFileLoggerManager.getInstance.insertLog(this, ITMFileLoggerVarsConsts.logSource.OUCH, ITMFileLoggerVarsConsts.logLevel.ERROR, ex0);
